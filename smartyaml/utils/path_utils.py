...
def resolve_path(path: Union[str, Path], base_path: Path, allow_absolute: Optional[bool] = None) -> Path:
    """
    ...
    try:
        ...
        if not allow_absolute:
            raise InvalidPathError(f"Path '{path}' resolves outside the base directory: {resolved}")
        
        # For absolute paths, perform additional security checks
        if path.is_absolute():
            if config.is_path_restricted(resolved):
                raise InvalidPathError(f"Access to system path '{resolved}' is not allowed")
        else:
            raise InvalidPathError(f"Relative path '{path}' resolves outside base directory: {resolved}")
