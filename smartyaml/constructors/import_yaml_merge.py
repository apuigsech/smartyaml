...
class ImportYamlMergeConstructor(FileBasedConstructor):
    ...
    def __call__(self, loader, tag_suffix, node):
        ...
        try:
            ...
            context = self.build_error_context(loader, 
    ...
    def extract_parameters_with_suffix(self, loader, tag_suffix, node) -> Dict[str, Any]:
        ...
        if isinstance(node, yaml.ScalarNode):
            ...
            if tag_suffix:
                ...
                if filename is None:
                    raise SmartYAMLError(f"{self.directive_name} mapping requires a filename")
        else:
            raise SmartYAMLError(f"{self.directive_name} expects a scalar filename or mapping with overrides")
