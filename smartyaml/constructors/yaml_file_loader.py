...
class YamlFileLoaderMixin:
    ...
    def load_yaml_file(
        ...
        if hasattr(parent_loader, 'accumulated_vars') and parent_loader.accumulated_vars:
        ...
        if self._contains_inline_template(yaml_content):
            ...
            if isinstance(result, dict) and '__template' in result:
                ...
                template_variables = getattr(parent_loader, 'accumulated_vars', {}).copy()
                ...
                result = template_processor.process_inline_template(result, template_context)
        else:
