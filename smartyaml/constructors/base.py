...
from typing import TYPE_CHECKING, Any, Dict, Optional, Union, List, Callable
from ..exceptions import SmartYAMLError, ConstructorError
...
if TYPE_CHECKING:
    from ..type_annotations import (
        YAMLLoader, YAMLNode, ParameterDict, ContextDict,
        LoaderContextType
    )
    ...
    def __call__(self, loader: yaml.SafeLoader, node: yaml.Node) -> Any:
        ...
        try:
            ...
            if self.parameter_extractor:
                params = self.parameter_extractor.extract_parameters(loader, node, self.directive_name)
            ...
            if self.type_converter and hasattr(self, 'TYPE_SPECS'):
                params = self.type_converter.convert_parameters(params, self.TYPE_SPECS, self.directive_name)
    ...
    def validate_parameters(self, params: Dict[str, Any]) -> None:
        ...
        if 'var_name' in params:
            ...
            if not var_name or not isinstance(var_name, str):
                raise ConstructorError(f"{self.directive_name} requires a non-empty variable name")
...
class ConditionalConstructor(FileBasedConstructor):
    ...
    def validate_parameters(self, params: Dict[str, Any]) -> None:
        ...
        if 'condition' not in params or 'filename' not in params:
            raise ConstructorError(f"{self.directive_name} requires both condition and filename")
        
        condition = params['condition']
        filename = params['filename']
        
        if not condition or not isinstance(condition, str):
            raise ConstructorError(f"{self.directive_name} condition must be a non-empty string")
        if not filename or not isinstance(filename, str):
            raise ConstructorError(f"{self.directive_name} filename must be a non-empty string")
