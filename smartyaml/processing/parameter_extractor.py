...
class ParameterPattern(Enum):
    ...
    FLEXIBLE_SEQUENCE = "flexible_sequence"  # !tag(value1, ..., valueN) - variable count
...
class ParameterExtractor:
    ...
    def extract_parameters(self, loader: yaml.SafeLoader, node: yaml.Node, 
                          directive_name: str) -> Dict[str, Any]:
        """
    ...
    def _extract_single_scalar(self, loader: yaml.SafeLoader, node: yaml.Node) -> Dict[str, Any]:
        """Extract a single scalar parameter."""
        if len(self.ordered_specs) != 1:
            raise ValueError("SINGLE_SCALAR pattern requires exactly one parameter spec")
    ...
    def _extract_scalar_or_sequence(self, loader: yaml.SafeLoader, node: yaml.Node) -> Dict[str, Any]:
        """Extract parameters that can be either scalar or sequence."""
        if isinstance(node, yaml.ScalarNode):
            if len(self.ordered_specs) < 1:
                raise ValueError("SCALAR_OR_SEQUENCE pattern requires at least one parameter spec")
    ...
    def _extract_fixed_sequence(self, loader: yaml.SafeLoader, node: yaml.Node, 
                               directive_name: str) -> Dict[str, Any]:
    ...
    def _extract_flexible_sequence(self, loader: yaml.SafeLoader, node: yaml.Node) -> Dict[str, Any]:
    ...
    def _extract_mapping(self, loader: yaml.SafeLoader, node: yaml.Node) -> Dict[str, Any]:
