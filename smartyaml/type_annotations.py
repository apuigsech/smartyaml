...
from typing import (
    TYPE_CHECKING, Any, Dict, List, Optional, Union, Callable, 
    Protocol, TypeVar, Generic, Literal, overload
)
...
class ExtractorProtocol(Protocol):
    ...
    def extract_parameters(self, loader: YAMLLoader, node: YAMLNode, 
                          directive_name: str) -> ParameterDict:
        """Extract parameters from a YAML node."""
        ...

# Literal types for better type safety
DirectiveName = Literal[
    '!import', '!import_yaml', '!env', '!include_if', '!include_yaml_if',
    '!template', '!base64', '!base64_decode'
]

ParameterPattern = Literal[
    'single_scalar', 'scalar_or_sequence', 'fixed_sequence', 
    'flexible_sequence', 'mapping', 'complex'
]
...
def is_yaml_mapping_node(node: Any) -> bool:
    ...
    return hasattr(node, 'value') and isinstance(node.value, list) and \
           all(isinstance(item, tuple) and len(item) == 2 for item in node.value)
...
class TypedConstructorRegistry:
    ...
    def dispatch(self, loader: YAMLLoader, tag_suffix: str, 
                node: YAMLNode) -> Any:
...
LoaderContextType = Dict[Literal['base_path', 'template_path', 'max_file_size', 
                               'max_recursion_depth', 'import_stack'], Any]

ParameterSpecType = Dict[Literal['name', 'type', 'required', 'default', 
