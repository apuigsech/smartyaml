...
class ConstructorError(SmartYAMLError):
    ...
    def __init__(self, directive_name: Optional[str] = None, 
        ...
        enhanced_message = self._build_message(directive_name, parameter_name, message, location)
        super().__init__(enhanced_message)
    
    def _build_message(self, directive_name: Optional[str], parameter_name: Optional[str], 
                      message: str, location: Optional[yaml.Mark]) -> str:
...
class ErrorMessages:
    ...
    INVALID_PARAMETER_TYPE = "Parameter '{param}' must be {expected_type}, got {actual_type}"
    ...
    ENVIRONMENT_VAR_NOT_FOUND = "Environment variable '{var_name}' not found and no default provided"
    ENVIRONMENT_VAR_INVALID = "Environment variable '{var_name}' has invalid value: {reason}"
    
    RECURSION_LIMIT_EXCEEDED = "Maximum recursion depth ({limit}) exceeded. Import chain: {chain}"
...
class ErrorFactory:
    ...
    @staticmethod
    def parameter_required(directive_name: str, param_name: str, 
                          location: Optional[yaml.Mark] = None) -> ConstructorError:
    ...
    @staticmethod
    def parameter_type_mismatch(directive_name: str, param_name: str,
    ...
    @staticmethod
    def parameter_validation_failed(directive_name: str, param_name: str, 
                                   details: str, location: Optional[yaml.Mark] = None) -> ConstructorError:
    ...
    @staticmethod
    def file_not_found(directive_name: str, filepath: str, 
                      reason: str = "File not found") -> SmartYAMLFileNotFoundError:
    ...
    @staticmethod
    def environment_variable_error(directive_name: str, var_name: str, 
                                  reason: str = "not found") -> EnvironmentVariableError:
        """Create error for environment variable issues."""
        if reason == "not found":
            message = ErrorMessages.ENVIRONMENT_VAR_NOT_FOUND.format(var_name=var_name)
        else:
            message = ErrorMessages.ENVIRONMENT_VAR_INVALID.format(var_name=var_name, reason=reason)
    ...
    @staticmethod
    def recursion_limit_exceeded(directive_name: str, limit: int, 
                               import_chain: str) -> RecursionLimitError:
    ...
    @staticmethod
    def encoding_error(directive_name: str, operation: str, 
                      details: str) -> Base64Error:
