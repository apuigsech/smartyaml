...
class SmartYAMLConfig:
    ...
    restricted_paths: Set[str] = field(default_factory=lambda: {
        '/etc/', '/proc/', '/sys/', '/dev/',  # Unix system paths
        'c:\\windows\\', 'c:\\program files\\',  # Windows system paths
    })
    ...
    truthy_values: Set[str] = field(default_factory=lambda: {
    ...
    def is_path_restricted(self, path: Path) -> bool:
        ...
        return any(path_str.startswith(restricted) for restricted in self.restricted_paths)
