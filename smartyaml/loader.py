    ...
    def flatten_mapping(self, node):
        ...
        while index < len(node.value):
            ...
            if key_node.tag == 'tag:yaml.org,2002:merge':
                ...
                if value_node.tag == 'tag:yaml.org,2002:merge':
                ...
                elif value_node.tag in ('tag:yaml.org,2002:map', 'tag:yaml.org,2002:omap'):
                    ...
                    for subnode in value_node.value:
                        if subnode.tag not in ('tag:yaml.org,2002:map', 'tag:yaml.org,2002:omap'):
                            raise yaml.constructor.ConstructorError(None, None,
                                'expected a mapping for merging, but found %s' % subnode.tag,
                                subnode.start_mark)
                    ...
                    if hasattr(value_node, 'tag') and value_node.tag and value_node.tag.startswith('!'):
                        try:
                            ...
                            if value_node.tag in self.yaml_constructors:
                                ...
                                merge_value = smart_constructor_dispatch(self, tag_suffix, value_node)
                            ...
                            if isinstance(merge_value, dict):
                                ...
                                for k, v in merge_value.items():
                                    key_node = yaml.ScalarNode('tag:yaml.org,2002:str', str(k))
                                    val_node = self.create_value_node(v, value_node.start_mark)
                                    pairs.append((key_node, val_node))
                                mapping_node = yaml.MappingNode('tag:yaml.org,2002:map', pairs)
                                merge.append(mapping_node)
                            else:
                                # Not a mapping, this will cause an error
                                raise yaml.constructor.ConstructorError(None, None,
                                    'expected a mapping for merging, '
                                    'but SmartYAML directive returned %s' % type(merge_value).__name__,
                                    value_node.start_mark)
                        except Exception as e:
                            # Re-raise as ConstructorError for consistency
                            raise yaml.constructor.ConstructorError(None, None,
                                'failed to construct SmartYAML directive for merging: %s' % str(e),
                                value_node.start_mark)
                    else:
                        # Not a SmartYAML directive, use original error
                        raise yaml.constructor.ConstructorError(None, None,
                            'expected a mapping for merging, but found %s' % value_node.tag,
                            value_node.start_mark)
            else:
                index += 1
